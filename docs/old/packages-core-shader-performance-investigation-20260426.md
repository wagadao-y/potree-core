...
| 7. EDL projection matrix uniform 更新時の allocation 削減 | 採用 | `uProj` 用 `Float32Array(16)` を material 内で再利用し、uniform 参照の差し替えをやめた | EDL path の per-frame allocation を削減。GPU ではなく CPU / GC 側の安定化策として維持 |
...
- 2026-04-26 EDL projection matrix uniform の allocation 削減を追加実装
...
シェーダー改善まわりの low-risk な項目は一通り消化できた。ここから先は、シェーダー micro-optimization よりも、GPU に送る点数と draw call 数を直接減らす施策を主戦場にする。

1. `docs/packages-core-performance-strategy-20260425.md` の方針どおり、dynamic point budget と Screen-Space Density LOD の運用化を優先する
   - steady-state FPS は GPU time と submitted points に強く支配されるため、以後の改善余地は shader ALU より point 数制御側にある。
2. clip-aware な可視制御の拡張を進める
   - clipping 有効時の ROI は shader より CPU 側の visible / budget 制御の方が大きい。
3. シェーダー側は、今後 regressions を避けるための定点観測を維持する
   - `JS heap`、`GPU time`、`submitted points`、`Generated bytes / point` を同条件で比較する。

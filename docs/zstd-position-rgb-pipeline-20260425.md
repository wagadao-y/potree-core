...
現在 worker は各 node で `0..numPoints-1` の `Uint32Array` を生成している。

```text
indices: 4 bytes/point
```

`THREE.Points` で index が不要なら、33M points 表示時で約 `132 MB` の JS heap / transfer 削減になる。`position+rgb` データでは残る大きな派生バッファが `position`, `rgba`, `indices` に絞られるため、次に確認する価値が高い。

検証項目:

- index attribute を作らずに描画・pick・shader が動くか。
- `PointAttribute.INDICES` を参照している shader / picker / material path があるか。
- 削除時の `Transfer avg`, `JS heap`, `Pick` 動作。

### 2. worker 返送 payload の削減
...
### 3. transfer / generated buffer bytes の計測追加
...
これにより、属性削減や indices 削除の効果を数値で追える。

### 4. position+rgb 専用 fast path
...
1. `indices` 生成の削除可否確認
2. worker 返送 payload から未使用 `buffer` を削除
3. transfer / generated buffer bytes の計測追加
4. `position+rgb` 専用 fast path
5. Zstd decoder warmup
6. `maxBytesInFlight`

...
export const getFeatures = () => ({
  SHADER_INTERPOLATION:
    hasExtension("EXT_frag_depth") && hasMinVaryingVectors(8),
  SHADER_SPLATS:
    hasExtension("EXT_frag_depth") &&
    hasExtension("OES_texture_float") &&
    hasMinVaryingVectors(8),
...
function getPrecision() {
  ...
  const vsHighpFloat = gl.getShaderPrecisionFormat(
    gl.VERTEX_SHADER,
    gl.HIGH_FLOAT,
  );
  const vsMediumpFloat = gl.getShaderPrecisionFormat(
    gl.VERTEX_SHADER,
    gl.MEDIUM_FLOAT,
  );

  const fsHighpFloat = gl.getShaderPrecisionFormat(
    gl.FRAGMENT_SHADER,
    gl.HIGH_FLOAT,
  );
  const fsMediumpFloat = gl.getShaderPrecisionFormat(
    gl.FRAGMENT_SHADER,
    gl.MEDIUM_FLOAT,
  );
  ...
  const highpAvailable =
    vsHighpFloat &&
    fsHighpFloat &&
    vsHighpFloat.precision > 0 &&
    fsHighpFloat.precision > 0;

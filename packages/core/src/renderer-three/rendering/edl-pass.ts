...
function isPerspectiveOrOrthographicCamera(
  camera: Camera,
): camera is PerspectiveCamera | OrthographicCamera {
  return (
...
export class EDLPass {
  ...
  private ensureRenderTargetType(renderer: WebGLRenderer): void {
    ...
    const hasColorBufferHalfFloat = Boolean(
  ...
  public render(params: EDLPassParams): void {
    ...
    if (!isPerspectiveOrOrthographicCamera(camera)) {
      throw new Error(
        "EDLPass.render: camera must be PerspectiveCamera or OrthographicCamera",
      );
    }

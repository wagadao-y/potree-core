...
onmessage = function (event) {
  ...
  for (const pointAttribute of pointAttributes.attributes) {
    if (pointAttribute.name === Potree.PointAttribute.POSITION_CARTESIAN.name) {
      ...
      pointAttribute.name === Potree.PointAttribute.COLOR_PACKED.name
    ) {
      ...
      for (let j = 0; j < numPoints; j++) {
        const r =
          cv.getUint16(inOffset + j * pointAttributes.byteSize + 0) / div;
        const g =
          cv.getUint16(inOffset + j * pointAttributes.byteSize + 2) / div;
        const b =
          cv.getUint16(inOffset + j * pointAttributes.byteSize + 4) / div;
      ...
      for (let j = 0; j < numPoints; j++) {
        const classification = cv.getUint8(
          inOffset + j * pointAttributes.byteSize,
        );

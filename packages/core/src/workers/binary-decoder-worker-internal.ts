...
function decodePointAttribute(
  attribute: IPointAttribute,
  ctx: Ctx,
): DecodedAttribute | undefined {
...
function decodePositionCartesian(
  attribute: IPointAttribute,
  ctx: Ctx,
): DecodedAttribute {
  ...
  for (let i = 0; i < ctx.numPoints; i++) {
    ...
    if (ctx.version.newerThan("1.3")) {
      x =
        ctx.data.getUint32(
          ctx.currentOffset + i * ctx.pointAttributes.byteSize + 0,
        ) * ctx.scale;
      y =
        ctx.data.getUint32(
          ctx.currentOffset + i * ctx.pointAttributes.byteSize + 4,
        ) * ctx.scale;
      z =
        ctx.data.getUint32(
          ctx.currentOffset + i * ctx.pointAttributes.byteSize + 8,
        ) * ctx.scale;
    } else {
      x =
        ctx.data.getFloat32(i * ctx.pointAttributes.byteSize + 0) +
        ctx.nodeOffset[0];
      y =
        ctx.data.getFloat32(i * ctx.pointAttributes.byteSize + 4) +
        ctx.nodeOffset[1];
      z =
        ctx.data.getFloat32(i * ctx.pointAttributes.byteSize + 8) +
        ctx.nodeOffset[2];
    }
...
function decodeColor(attribute: IPointAttribute, ctx: Ctx): DecodedAttribute {
  ...
  for (let i = 0; i < ctx.numPoints; i++) {
    colors[3 * i + 0] = ctx.data.getUint8(
      ctx.currentOffset + i * ctx.pointAttributes.byteSize + 0,
    );
    colors[3 * i + 1] = ctx.data.getUint8(
      ctx.currentOffset + i * ctx.pointAttributes.byteSize + 1,
    );
    colors[3 * i + 2] = ctx.data.getUint8(
      ctx.currentOffset + i * ctx.pointAttributes.byteSize + 2,
    );
  }
...
function decodeIntensity(
  ...
  for (let i = 0; i < ctx.numPoints; i++) {
    intensities[i] = ctx.data.getUint16(
      ctx.currentOffset + i * ctx.pointAttributes.byteSize,
    );
  }
...
function decodeClassification(
  ...
  for (let j = 0; j < ctx.numPoints; j++) {
    classifications[j] = ctx.data.getUint8(
      ctx.currentOffset + j * ctx.pointAttributes.byteSize,
    );
  }
...
function decodeNormalSphereMapped(
  attribute: IPointAttribute,
  ctx: Ctx,
): DecodedAttribute {
  const buffer = new ArrayBuffer(ctx.numPoints * 4 * 3);
  ...
  for (let j = 0; j < ctx.numPoints; j++) {
    const bx = ctx.data.getUint8(
      ctx.currentOffset + j * ctx.pointAttributes.byteSize + 0,
    );
    const by = ctx.data.getUint8(
      ctx.currentOffset + j * ctx.pointAttributes.byteSize + 1,
    );
...
function decodeNormalOct16(
  ...
  return { buffer: buff, attribute: attribute };
}
...
function decodeNormal(attribute: IPointAttribute, ctx: Ctx): DecodedAttribute {
  ...
  for (let j = 0; j < ctx.numPoints; j++) {
    const x = ctx.data.getFloat32(
      ctx.currentOffset + j * ctx.pointAttributes.byteSize + 0,
    );
    const y = ctx.data.getFloat32(
      ctx.currentOffset + j * ctx.pointAttributes.byteSize + 4,
    );
    const z = ctx.data.getFloat32(
      ctx.currentOffset + j * ctx.pointAttributes.byteSize + 8,
    );

import type { IVisibilityUpdateResult, Potree } from "potree-core";
...
export class PotreeRenderer {
  ...
  private edlOptions: PotreeRendererEDLOptions = {
    enabled: false,
    pointCloudLayer: 1,
  };
  private lastPointClouds: PointCloudOctree[] | null = null;
  private overriddenMaterials = new WeakMap<
    PointCloudMaterial,
    { useEDL: boolean; weighted: boolean }
  >();
  ...
  public setEDL(options: PotreeRendererEDLOptions): void {
    ...
    this.edlOptions = {
      ...
      pointCloudLayer:
        options.pointCloudLayer ?? this.edlOptions.pointCloudLayer ?? 1,
    ...
    if (this.edlPass) {
      if (this.edlOptions.strength !== undefined)
        this.edlPass.edlStrength = this.edlOptions.strength;
      if (this.edlOptions.radius !== undefined)
        this.edlPass.radius = this.edlOptions.radius;
      if (this.edlOptions.opacity !== undefined)
        this.edlPass.opacity = this.edlOptions.opacity;
      if (this.edlOptions.neighbourCount !== undefined)
        this.edlPass.neighbourCount = this.edlOptions.neighbourCount;
    }
    ...
    for (const pco of pointClouds) {
      ...
      if (enabled) {
        ...
        if (!this.overriddenMaterials.has(material)) {
          this.overriddenMaterials.set(material, {
            useEDL: material.useEDL,
            weighted: material.weighted,
          });
        }
        ...
        if (prevMat) {
          if (material.useEDL !== prevMat.useEDL)
            material.useEDL = prevMat.useEDL;
          if (material.weighted !== prevMat.weighted)
            material.weighted = prevMat.weighted;
        }
